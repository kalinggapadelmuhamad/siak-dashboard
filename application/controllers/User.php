...
class User extends CI_Controller{
    ...
    public function createJurnal(){
		...
		if($role === 'owner'){
			...
			if(!$_POST){
				...
				// $saldo = substr($this->input->post('saldo',true), 4);
				// $saldo = trim(str_replace('.', '', $saldo));
				...
				//Loop melalui array dan sisipkan data ke dalam tabel
				for ($i = 0; $i < count($data['no_reff']); $i++) {
					// Ubah format saldo
					$saldo = substr($data['saldo'][$i], 4);
					$saldo = trim(str_replace('.', '', $saldo));

					$insert_data = array(
						'id_user'=>$id_user,
						'no_reff' => $data['no_reff'][$i],
						'tgl_input'=>$tgl_input,
						'tgl_transaksi' => $data['tgl_transaksi'],
						'jenis_saldo' => $data['jenis_saldo'][$i],
						'saldo' => $saldo
					);

					$this->jurnal->insertJurnal($insert_data);
    ...
    public function editForm(){
		...
		if($role === 'owner'){
			...
			if($_POST){
				$id = $this->input->post('id',true);
				$title = 'Edit'; $content = 'user/form_jurnal_edit'; $action = 'jurnal_umum/edit'; $titleTag = 'Edit Jurnal Umum';
