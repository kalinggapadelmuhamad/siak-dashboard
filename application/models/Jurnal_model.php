...
class Jurnal_model extends CI_Model{
	...
	public function getJurnalGroupByJenisSaldo($noReff) {
		return $this->db->select('transaksi.id_transaksi, transaksi.tgl_transaksi, akun.nama_reff, transaksi.no_reff, transaksi.jenis_saldo, SUM(transaksi.saldo) AS total_saldo, MAX(transaksi.tgl_input) AS max_tgl_input')
			->from($this->table)
			->join('akun', 'transaksi.no_reff = akun.no_reff')
			->group_by('transaksi.jenis_saldo')
			->order_by('tgl_transaksi', 'ASC')
			->get()
			->result();
	}
